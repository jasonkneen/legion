  ...
  return JSON.stringify(
    {
      ...
      theme_color: "#000000",
      icons: [
        // These live at the root of public/ and are the files that actually
        // ship. `/__grok/` is the middleware's dynamic namespace (manifest and
        // install page) and is gitignored — pointing icons there 404s.
        {
          src: "/icon-180.png",
          sizes: "180x180",
          type: "image/png",
...
export function injectGrokPwaHead(
  ...
  const missing = grokPwaHeadTags(appName)
    .filter(([key]) => {
      ...
      if (key === "apple-touch-icon") return !html.includes('href="/icon-180.png"');
      if (key === "twitter:card") {
        return (
