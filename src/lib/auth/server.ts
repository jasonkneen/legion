...
/**
 * Preview secret must outlive module reloads AND process restarts. Sessions and
 * provider keys live in on-disk PGLite; if this secret remints, every session
 * cookie dies and it looks like keys were never saved.
 */
  ...
  try {
    ...
    if (existsSync(path)) {
      ...
      if (existing.length >= 32) {
        globalAuthRef.__grokAuthPreviewSecret__ = existing;
        return existing;
      }
    }
    const next = randomBytes(32).toString("hex");
    writeFileSync(path, next, { encoding: "utf8", mode: 0o600 });
    globalAuthRef.__grokAuthPreviewSecret__ = next;
    return next;
  } catch {
    const next = randomBytes(32).toString("hex");
    globalAuthRef.__grokAuthPreviewSecret__ = next;
